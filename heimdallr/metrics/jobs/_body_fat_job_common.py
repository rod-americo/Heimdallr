#!/usr/bin/env python3
"""Shared helpers for abdominal VAT/SAT metric jobs."""
...
def compute_level_measurements(
    ...
    for level in TARGET_LEVELS:
        ...
        if extent is not None:
            ...
            vat_volume = slab_volume_cm3(vat_mask, spacing_xyz, extent)
            ratio = round(vat_volume / sat_volume, 4) if sat_volume > 0 else None
            payload.update(
                {
                    "slice_range": [int(extent[0]), int(extent[1])],
                    "sat_volume_cm3": sat_volume,
                    "vat_volume_cm3": vat_volume,
                    "vat_to_sat_ratio": ratio,
                    "subcutaneous_fat_volume_cm3": sat_volume,
                    "torso_fat_volume_cm3": vat_volume,
                    "visceral_proxy_volume_cm3": vat_volume,
...
def build_abdominal_aggregate(
    ...
    vat_volume = slab_volume_cm3(vat_mask, spacing_xyz, z_range)
    ratio = round(vat_volume / sat_volume, 4) if sat_volume > 0 else None
...
def save_l3_overlay(
    ...
    rotated_vat = np.rot90(np.asarray(vat_slice, dtype=bool))
    ...
    if rotated_vat.any():
        vat_masked = np.ma.masked_where(~rotated_vat, rotated_vat.astype(np.uint8))
        ax.imshow(vat_masked, cmap="autumn", alpha=0.42, interpolation="nearest")
        ax.contour(rotated_vat, levels=[0.5], colors=["#ffb000"], linewidths=1.1)

    if rotated_l3.any():
        ax.contour(rotated_l3, levels=[0.5], colors=["#00d5ff"], linewidths=0.9)

    ax.set_title("L3 VAT/SAT Reference", fontsize=14)
    ax.text(
        0.03,
        0.97,
        "\n".join(summary_lines),
...
def save_volumetry_profile(
    ...
    sat_values = [level_measurements[level]["sat_volume_cm3"] or 0.0 for level in labels]
    vat_values = [level_measurements[level]["vat_volume_cm3"] or 0.0 for level in labels]
    colors = [
    ...
    summary_lines = [
        ...
        f"VAT: {aggregate.get('vat_volume_cm3', '-')}",
        f"SAT: {aggregate.get('sat_volume_cm3', '-')}",
        f"Ratio: {aggregate.get('vat_to_sat_ratio', '-')}",
    ]
    ax0.axis("off")
    ax0.text(0.01, 0.95, "\n".join(str(line) for line in summary_lines), va="top", ha="left", family="monospace", fontsize=11)

    ax1.bar(x - width / 2, vat_values, width=width, label="VAT", color="#ef4444")
    ax1.bar(x + width / 2, sat_values, width=width, label="SAT", color=colors)
    ax1.set_xticks(x)
    ax1.set_xticklabels(labels)
    ax1.set_ylabel("Volume (cm3)")
    ax1.set_title("Abdominal VAT/SAT by vertebral slab")
    ax1.legend()
