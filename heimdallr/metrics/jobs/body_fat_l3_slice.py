#!/usr/bin/env python3
"""Measure VAT/SAT compartments on the center slice of L3."""
...
def main() -> int:
    ...
    try:
        ...
        vat_path = inputs["torso_fat_path"]
        ...
        payload["inputs"] = {
            ...
            "sat_mask": str(sat_path.relative_to(case_dir)) if sat_path.exists() else None,
            "vat_mask": str(vat_path.relative_to(case_dir)) if vat_path.exists() else None,
            # Legacy aliases
            "subcutaneous_fat_mask": str(sat_path.relative_to(case_dir)) if sat_path.exists() else None,
            "torso_fat_mask": str(vat_path.relative_to(case_dir)) if vat_path.exists() else None,
        }

        if ct_path is None or not ct_path.exists() or not sat_path.exists() or not vat_path.exists() or not l3_path.exists():
        ...
        _, vat_mask = load_nifti_mask(vat_path)
        _, l3_mask = load_nifti_mask(l3_path)
        if not (ct_data.shape == sat_mask.shape == vat_mask.shape == l3_mask.shape):
            payload["status"] = "skipped"
        ...
        vat_slice = vat_mask[:, :, slice_index]
        ...
        vat_area = slice_area_cm2(vat_slice, spacing_xy)
        ...
        if job_config.get("generate_overlay", True):
            save_l3_overlay(
                ...
                summary_lines=[
                    ...
                    f"SAT area: {sat_area:.1f} cm2",
                    f"VAT area: {vat_area:.1f} cm2",
        ...
        payload = {
            ...
            "measurement": {
                ...
                "torso_fat_area_cm2": vat_area,
                "visceral_proxy_area_cm2": vat_area,
