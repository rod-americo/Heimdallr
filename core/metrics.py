...
def resolve_artifact_dir(case_output_folder, name):
    """Support both flat and artifacts/ nested study layouts."""
    candidates = [
        case_output_folder / name,
        case_output_folder / "artifacts" / name,
    ]
...
def resolve_metadata_file(case_output_folder, filename):
    """Support both flat and metadata/ nested study layouts."""
    candidates = [
        case_output_folder / filename,
        case_output_folder / "metadata" / filename,
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
...
def ensure_metric_artifact_dir(case_output_folder, metric_name):
    """Create a metrics artifact directory compatible with both layouts."""
    if (case_output_folder / "artifacts").exists():
        artifact_dir = case_output_folder / "artifacts" / "metrics" / metric_name
    else:
        artifact_dir = case_output_folder / "metrics" / metric_name
...
def calculate_all_metrics(case_id, nifti_path, case_output_folder, generate_overlays=True):
    ...
    """
    total_dir = resolve_artifact_dir(case_output_folder, "total")
    tissue_types_dir = resolve_artifact_dir(case_output_folder, "tissue_types")
    ...
    id_json_path = resolve_metadata_file(case_output_folder, "id.json")
    modality = "CT"
    kvp_value = None
    kvp_raw = "Unknown"
    ...
    muscle_file = tissue_types_dir / "skeletal_muscle.nii.gz"
