...
def build_case_report(case_folder: Path, output_path: Path | None = None) -> Path:
    ...
    if output_path is None:
        output_path = case_folder / "report.pdf"

    metadata = _safe_json(case_folder / "id.json")
    results = _safe_json(case_folder / "resultados.json")
    triage_report = _safe_json(case_folder / "urology" / "kidney_stone_triage.json")
    ...
    summary_items = [
        ("Patient", metadata.get("PatientName", "-")),
        ...
        ("T Pre-Proc", pipeline.get("prepare_elapsed_time", "-")),
        ("T Proc", pipeline.get("processing_elapsed_time") or pipeline.get("elapsed_time", "-")),
    ]
    summary_cols = 4
    ...
    _draw_card(draw, "Technical Timing", [
        ...
        ("Processing", pipeline.get("processing_elapsed_time") or pipeline.get("elapsed_time", "-")),
