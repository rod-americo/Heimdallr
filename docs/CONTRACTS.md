...
| Pleural/pericardial effusion overlay DICOM series | `artifacts/metrics/pleural_pericard_effusion/*_component_*.dcm` | DICOM Secondary Capture series | Positive axial mediastinal-window overlays (`WL 40 HU`, `WW 400 HU`), one image per connected component of each present finding. Pleural and pericardial components share one series and are linked to their finding and component identifiers. No result or presentation artifact is emitted when both masks are empty. |
