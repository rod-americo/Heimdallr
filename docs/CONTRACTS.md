...
| QC evidence artifacts | `evidence/<analysis_id>/acquisitions/<acquisition_id>/` | `total_multilabel.nii.gz`, logs, and anatomy-state JSON | Created only when QC resolves enabled. One multilabel image is retained per segmented acquisition; voxel counts, volumes, and anatomy bounds are not part of this contract. The artifacts are independent from metrics and DICOM egress. |
...
| Head brain-geometry 2 mm artifact | `artifacts/metrics/head_complete_qc/normalized_brain_geometry_head_ct_2mm.nii.gz` | NIfTI | Volume resampled while preserving the selected source acquisition's axial-plane normal. `total/brain.nii.gz` is projected into that plane and a two-dimensional PCA defines only the right-left/anterior-posterior rotation. No `brain_structures` mask or orbitomeatal landmark participates in the frame. Source in-plane spacing is preserved by default and slice spacing is 1 mm. |
...
| Hepatic lesion result | `artifacts/metrics/liver_lesions/result.json` | JSON | Result from the TotalSegmentator `liver_lesions` task. The API-facing signal is `measurement.has_hepatic_lesion`; connected components include voxel counts, volumes, centroids, bounding boxes, and representative axial positions. A component is eligible only when at least 50% of its voxels intersect the final `total/liver.nii.gz` mask and its maximum boundary-to-boundary axial Feret diameter is at least 4 mm. An eligible partially overlapping component retains its complete predicted extent. `measurement.anatomical_qc` preserves the 0.5 overlap threshold, raw, eligible, and excluded component/voxel counts, each overlap fraction, the axial diameter, and the per-component decision. The automatic CT planner requests the model only when `total/liver.nii.gz` is positive and the host profile enables both the task and job. |
...
mask centroid as the frame center, preserves the source acquisition axial-plane
normal, and uses a two-dimensional PCA of the mask projection to define only
the in-plane right-left/anterior-posterior rotation. `total/skull.nii.gz` may
define the crop field of view but never the axes. This is the preferred head
geometry artifact when an exact orbitomeatal line is not required.
