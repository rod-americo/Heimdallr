...
| Pulmonary nodule overlay DICOM | `artifacts/metrics/lung_nodules/overlay_sc.dcm` | DICOM Secondary Capture | Positive-case axial overlay for the `lung_nodules` mask. Emitted only when at least one positive nodule voxel is present. |
...
positive voxels. Positive cases emit a Secondary Capture overlay; negative
cases emit only result JSON.
