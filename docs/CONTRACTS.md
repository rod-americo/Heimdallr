...
| Pulmonary nodule result | `artifacts/metrics/lung_nodules/result.json` | JSON | Produced by the `lung_nodules` metrics job when the TotalSegmentator `lung_nodules` task is available. The API-facing signal is `measurement.has_pulmonary_nodule`, set from whether any nodule mask has positive voxels. |
| Pulmonary nodule overlay DICOM series | `artifacts/metrics/lung_nodules/nodule_component_*.dcm` | DICOM Secondary Capture series | Positive-case axial lung-window overlays (`WL -600 HU`, `WW 1500 HU`) for the `lung_nodules` mask, one DICOM image per connected nodule component. Each component is linked by `component_id` in `measurement.components`, `artifacts.component_overlays`, and `dicom_exports`; the legacy `artifacts.overlay_sc_dcm` field points to the first component image for compatibility. DICOM image-plane position is copied from the nearest preserved source CT instance for position-based synchronization. Emitted only when at least one positive nodule voxel is present. |
| Pleural/pericardial effusion result | `artifacts/metrics/pleural_pericard_effusion/result.json` | JSON | Positive-only result from the TotalSegmentator `pleural_pericard_effusion` task. Each present finding includes `voxel_count`, total `volume_cm3`, and connected components with individual volumes. Pleural components are assigned to the nearest segmented left or right lung surface in physical space; the payload exposes `left_volume_cm3`, `right_volume_cm3`, and `indeterminate_volume_cm3`. It is written and published under `metrics.pleural_pericard_effusion` only when at least one pleural or pericardial effusion mask has positive voxels. |
| Pleural/pericardial effusion overlay DICOM series | `artifacts/metrics/pleural_pericard_effusion/overlay_*.dcm` | DICOM Secondary Capture series | Positive axial 5 mm slab-average overlays in a mediastinal window (`WL 40 HU`, `WW 400 HU`). Every slab intersecting either positive mask is exported, so the series covers the complete pleural and pericardial effusion extent. Both findings are shown together when they intersect the same slab, with lateralized pleural and pericardial volumes in cm³. DICOM image-plane position is copied from the nearest preserved source CT instance. No result or presentation artifact is emitted when both masks are empty. |
